...
def delete_data(filter_name=None, filter_phone=None):
    try:
        conn = connect_to_database()
        if conn:
            cur = conn.cursor()
            if filter_name and filter_phone:
                cur.execute("DELETE FROM PHONEBOOK WHERE HUMAN_NAME = %s AND PHONENUMBER = %s;", (filter_name, filter_phone))
            elif filter_name:
                cur.execute("DELETE FROM PHONEBOOK WHERE HUMAN_NAME = %s;", (filter_name,))
            elif filter_phone:
                cur.execute("DELETE FROM PHONEBOOK WHERE PHONENUMBER = %s;", (filter_phone,))
            else:
                print("Не указаны фильтры для удаления.")
                return
            conn.commit()
...
# Вызов функции удаления данных по имени пользователя или номеру телефона
delete_data(filter_name='John Doe')  # Пример удаления по имени пользователя
delete_data(filter_phone='1234567890')  # Пример удаления по номеру телефона
